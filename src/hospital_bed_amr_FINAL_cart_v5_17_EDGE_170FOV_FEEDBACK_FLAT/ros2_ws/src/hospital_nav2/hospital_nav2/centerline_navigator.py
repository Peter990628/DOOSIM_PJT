...
class CenterlineNavigator(Node):
    ...
    def __init__(self) -> None:
        ...
        self.require_pose_lock = bool(self.get_parameter("require_pose_lock").value)
        ...
        self.max_follow_path_retries = max(0, int(self.get_parameter("max_follow_path_retries").value))
        ...
        if self.require_pose_lock:
            self.create_subscription(Bool, self.pose_lock_topic, self._on_pose_lock, map_qos)
        else:
            # Trolley localization currently comes from the Isaac ground-truth TF
            # bridge, so there is no separate /initial_pose_locked publisher.
        ...
        self.get_logger().info(
            ...
            f"회전 progress 제약=OFF, FollowPath 자동 재시도={'무제한' if self.max_follow_path_retries == 0 else self.max_follow_path_retries}회"
        )
    ...
    def _on_goal(self, msg: PoseStamped) -> None:
        if self.require_pose_lock and not self.pose_locked:
    ...
    def _plan_goal(self, msg: PoseStamped, is_retry: bool = False) -> None:
        ...
        self.final_yaw = yaw_from_quaternion(q.x, q.y, q.z, q.w)
    ...
    def _result_callback(self, future) -> None:
        ...
        self.segment_rotation_done = False
        if self.segment_index >= len(self.segments):
            self.get_logger().info("마지막 직선 구간 도착: 최종 yaw를 별도 회전으로 맞춥니다.")
            self.next_segment_time = time.monotonic() + self.corner_stop_sec
        else:
            self.get_logger().info("코너 도착: 완전 정지 후 다음 방향으로 회전합니다.")
            self.next_segment_time = time.monotonic() + self.corner_stop_sec
