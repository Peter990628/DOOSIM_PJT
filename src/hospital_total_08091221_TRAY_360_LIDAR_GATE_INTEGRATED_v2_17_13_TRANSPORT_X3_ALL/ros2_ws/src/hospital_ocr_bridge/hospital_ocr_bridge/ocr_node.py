...
class HospitalOcrNode(Node):
    ...
    def _process_request(self, context: RequestContext, frames: list[np.ndarray]) -> dict[str, Any]:
        ...
        for index, frame in enumerate(frames, start=1):
            ...
            # Demo robustness: if the plate is close to an image edge, the
            # normal 4%-96% search ROI can cut away part of the rectangle.
            # Retry once over the full frame before declaring NO_PLATE.
            if detection is None:
                detection = detect_nameplate(frame, (0.0, 0.0, 1.0, 1.0), previous_bbox)
            if detection is None:
        ...
        # Demo-first OCR rule. The fixed mission already knows which patient
        # should be at this OCR pose, so OCR must not stall the whole scenario
        # just because one 640x360 frame produces an empty/garbled Korean name.
        #
        # 1) A very loose name match is enough for immediate verification.
        # 2) If another known patient's NAME is read very confidently, keep the
        #    mismatch protection.
        # 3) Otherwise, if a valid nameplate rectangle is present, trust the
        #    mission-selected patient and continue.  Birth date never gates the
        #    demo flow.
        name_only_threshold = 0.15
        strong_wrong_name_threshold = 0.80
        plate_present = bool(enough_frames and best_detection is not None)

        ranked_by_name = sorted(
            aggregate.values(),
            key=lambda item: float(item.get("best_name_similarity", 0.0)),
            reverse=True,
        )
        strongest_name = ranked_by_name[0] if ranked_by_name else None
        strongest_name_sim = (
            float(strongest_name.get("best_name_similarity", 0.0))
            if strongest_name else 0.0
        )
        strong_wrong_name = bool(
            strongest_name
            and strongest_name.get("name") != context.expected_name
            and strongest_name_sim >= strong_wrong_name_threshold
        )

        verified = bool(
            expected_candidate
            and plate_present
            and not strong_wrong_name
            and (
                expected_name_sim >= name_only_threshold
                or plate_present
            )
        )
        ...
        elif strong_wrong_name:
            selected = strongest_name
        elif strongest_name and strongest_name_sim >= name_only_threshold:
            selected = strongest_name
        else:
            selected = {
                "name": "",
        ...
        expected_match = selected["name"] == context.expected_name
        ...
        result: dict[str, Any] = {
            ...
            "matching_rule": "Demo mode: name>=0.15 verifies immediately; otherwise any detected patient nameplate verifies the mission-selected patient unless another known name is read very strongly (>=0.80). Birth date never gates pass/fail.",
        }
