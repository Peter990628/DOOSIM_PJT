...
# AMR1이 1F로 복귀했을 때, 출발 대기 중인 AMR2에 좁은 복도/엘리베이터
# 예약을 넘기기 위한 넓은 로비의 안전 대기점이다. 이 좌표는 정적 지도에서
# 가장 가까운 벽까지 약 2.70 m, 1F 엘리베이터 목표까지 약 4.09 m 떨어져 있다.
RETURN_HOLD_1F = {
    "x": -29.724452517267892,
    "y": 19.387989248988994,
    "yaw": -1.58135670468565,
}
# AMR1 병실 복귀 시 긴 침대가 골목 옆 벽을 스치지 않도록, 넓은 공간에서
# 골목의 첫 직선 구간과 평행하게 자세를 먼저 고정하는 전용 waypoint다.
# 실측 yaw와 이 점에서 생성되는 첫 경로 yaw의 차이는 약 0.44도다.
ROOM_RETURN_ENTRY_WAYPOINT_1F = {
    "x": -29.543911270032343,
    "y": 12.400311173370898,
    "yaw": 3.1362923925236132,
}
RETURN_HANDOFF_TIMEOUT_S = 240.0
ROUTE_RESERVATION_PRIORITY = {
    "TO_MRI_LOADED": 200,
    "FROM_MRI_LOADED": 100,
}
MRI_WAIT_S = 3.0
...
ROUTE_RESERVATION_STATUS_TOPIC = "/traffic_reservation/status"
...
class KimSeoulMission(Node):
    def __init__(self, patient: dict, amr_id: str = "amr1") -> None:
        ...
        self.route_reservation_request_id = ""
        self.route_reservation_phase = ""
        self.route_reservation_priority = 0
        self.route_reservation_granted = False
        self.route_reservation_last_detail = ""
        self.route_reservation_status: dict = {}
        self.elevator_status_by_robot: dict[str, dict] = {}
        ...
        self.create_subscription(
            String,
            ROUTE_RESERVATION_STATUS_TOPIC,
            self._on_route_reservation_status,
            latched,
        )
        ...
        self.route_reservation_pub = self.create_publisher(
            String, f"/{self.amr_id}/route_reservation/request", 20
        )
        self.create_subscription(
            String,
            f"/{self.amr_id}/route_reservation/grant",
            self._on_route_reservation_grant,
            latched,
        )
    ...
    def _publish_special_motion_state(self) -> None:
        ...
        if self.route_reservation_request_id:
            self._publish_route_reservation("HEARTBEAT")

    def _publish_route_reservation(self, action: str) -> None:
        if not self.route_reservation_request_id:
            return
        msg = String()
        msg.data = json.dumps(
            {
                "robot": self.amr_id,
                "request_id": self.route_reservation_request_id,
                "resource": "LOADED_TRANSPORT_ROUTE",
                "action": str(action).upper(),
                "phase": self.route_reservation_phase,
                "priority": self.route_reservation_priority,
                "timestamp": time.time(),
            },
            ensure_ascii=False,
            separators=(",", ":"),
        )
        self.route_reservation_pub.publish(msg)

    def _on_route_reservation_grant(self, msg: String) -> None:
        try:
            payload = json.loads(msg.data)
        except Exception:
            return
        if str(payload.get("request_id", "")) != self.route_reservation_request_id:
            return
        self.route_reservation_granted = bool(payload.get("granted", False))
        self.route_reservation_last_detail = str(payload.get("detail", ""))

    def _on_route_reservation_status(self, msg: String) -> None:
        try:
            self.route_reservation_status = json.loads(msg.data)
        except json.JSONDecodeError:
            return

    def _other_loaded_amr_waiting(self) -> bool:
        """Return whether AMR2 is queued behind AMR1's loaded return route."""
        if self.amr_id != "amr1":
            return False
        payload = self.route_reservation_status
        waiting = {
            str(robot).strip().lower()
            for robot in payload.get("waiting", [])
            if str(robot).strip()
        }
        return (
            str(payload.get("owner", "")).strip().lower() == "amr1"
            and str(payload.get("phase", "")).strip().upper() == "FROM_MRI_LOADED"
            and "amr2" in waiting
        )

    def _wait_for_amr2_up_handoff(self, timeout: float = RETURN_HANDOFF_TIMEOUT_S) -> str:
        """Wait at the 1F lobby until AMR2 has boarded and the car is going up.

        Returns ``CLEARED`` when AMR2 has physically left the shared 1F route,
        ``REACQUIRE`` when no AMR2 owner appeared before the timeout, and
        ``FAILED`` if AMR2 acquired the route but its elevator sequence failed or
        never reached a physically safe phase.
        """
        started_wall = time.time()
        end = time.monotonic() + max(1.0, float(timeout))
        other_owner_seen = False
        last_print = 0.0
        safe_up_states = {
            "RIDING_TO_2F",
            "RELOCKING_2F",
        ...
        print(
            "[1F 예약 인계 대기] AMR2가 예약을 받은 뒤 엘리베이터에 탑승해 "
            "2층으로 이동할 때까지 AMR1을 로비 대기점에 정지합니다."
        )
        while rclpy.ok() and time.monotonic() < end:
            rclpy.spin_once(self, timeout_sec=0.1)
            self.stop()

            reservation = self.route_reservation_status
            owner = str(reservation.get("owner", "")).strip().lower()
            phase = str(reservation.get("phase", "")).strip().upper()
            if owner == "amr2" and phase == "TO_MRI_LOADED":
                other_owner_seen = True

            elevator = self.elevator_status_by_robot.get("amr2", {})
            elevator_state = str(elevator.get("state", "")).strip().upper()
            elevator_direction = str(elevator.get("direction", "")).strip().lower()
            elevator_stamp = float(elevator.get("timestamp", 0.0) or 0.0)
            if (
                other_owner_seen
                and elevator_direction == "up"
                and elevator_stamp >= started_wall - 1.0
                and elevator_state in safe_up_states
            ):
                print(
                    f"[1F 예약 인계 완료] AMR2 상태={elevator_state}: "
                    "AMR2가 1층 공용 구간을 떠났으므로 AMR1 병실 복귀를 재개합니다."
                )
                return "CLEARED"

            if other_owner_seen and elevator_direction == "up" and elevator_state == "FAILED":
                print(
                    "[1F 예약 인계 실패] AMR2가 예약을 받은 뒤 엘리베이터 상승에 실패했습니다. "
                    "AMR1은 로비 안전 대기점에서 정지합니다."
                )
                return "FAILED"
            ...
            if now - last_print >= 3.0:
                print(
                    "[1F 예약 인계 상태] "
                    f"AMR2 owner={other_owner_seen}, elevator={elevator_state or 'WAIT'}"
                )
                last_print = now
        ...
        if other_owner_seen:
            print(
                "[1F 예약 인계 제한시간 초과] AMR2가 예약을 받았지만 2층 이동을 확인하지 못했습니다. "
                "예약을 강제로 빼앗지 않고 AMR1을 안전 대기점에 유지합니다."
            )
            return "FAILED"
        print(
            "[1F 예약 인계 취소] 제한시간 안에 AMR2가 예약 owner가 되지 않았습니다. "
            "AMR1이 정상 예약을 다시 요청합니다."
        )
        return "REACQUIRE"

    def acquire_loaded_route(self, phase: str) -> bool:
        phase = str(phase).strip().upper()
        if phase not in ROUTE_RESERVATION_PRIORITY:
            print(f"[공간 예약 실패] 지원하지 않는 단계: {phase}")
            return False
        self.route_reservation_request_id = (
            f"route-{self.amr_id}-{phase.lower()}-{uuid.uuid4().hex[:10]}"
        )
        self.route_reservation_phase = phase
        self.route_reservation_priority = ROUTE_RESERVATION_PRIORITY[phase]
        self.route_reservation_granted = False
        self.route_reservation_last_detail = ""
        self._publish_route_reservation("ACQUIRE")
        print(
            f"[공간 예약 요청] {self.amr_id.upper()} {phase}: "
            "1F 좁은 복도 + 엘리베이터 + 2F 공용 통로"
        )
        last_print = 0.0
        while rclpy.ok() and not self.route_reservation_granted:
            rclpy.spin_once(self, timeout_sec=0.1)
            self.stop()
            now = time.monotonic()
            if now - last_print >= 2.0:
                print(
                    f"[공간 예약 대기] {self.amr_id.upper()} {phase}: "
                    "현재 staging 위치에서 점유 AMR의 안전 해제까지 정지"
                )
                last_print = now
        if not rclpy.ok():
            return False
        print(
            f"[공간 예약 승인] {self.amr_id.upper()} {phase}: "
            f"{self.route_reservation_last_detail or 'exclusive route granted'}"
        )
        return True

    def release_loaded_route(self, safe_point: str) -> None:
        if not self.route_reservation_request_id:
            return
        request_id = self.route_reservation_request_id
        phase = self.route_reservation_phase
        # Repeat the explicit release so a single best-effort delivery hiccup cannot
        # leave the next mission waiting.  The coordinator validates owner+request_id.
        for _ in range(3):
            self._publish_route_reservation("RELEASE")
            rclpy.spin_once(self, timeout_sec=0.05)
        print(
            f"[공간 예약 해제] {self.amr_id.upper()} {phase}: "
            f"안전 지점={safe_point}, request_id={request_id}"
        )
        self.route_reservation_request_id = ""
        self.route_reservation_phase = ""
        self.route_reservation_priority = 0
    ...
    def _on_elevator_status(self, msg: String) -> None:
        try:
            ...
            return
        self.elevator_payload = payload
        robot = str(payload.get("robot", "")).strip().lower()
        if robot:
            self.elevator_status_by_robot[robot] = payload
    ...
    def switch_to_floor_map(self, floor: str, request_id: str) -> bool:
        ...
        if floor not in {"1f", "2f"}:
            ...
            return False
        # Resolve the map from the project that is actually running this mission.
        # This keeps copied/renamed project folders portable across PCs.
    ...
    def run(self) -> bool:
        ...
        print("[1F 공간예약] 병실 측 현재 staging 위치에서 공용 이송 경로를 요청합니다.")
        if not self.acquire_loaded_route("TO_MRI_LOADED"):
            return False
        ...
        self.release_loaded_route("MRI_INSPECTION_WAIT")
        ...
        if not self.acquire_loaded_route("FROM_MRI_LOADED"):
            return False
        ...
        # AMR2가 이미 침대를 결합하고 같은 예약을 기다리고 있을 때만 넓은 로비
        # 대기점으로 이동해 예약을 넘긴다. 대기 요청이 없으면 기존처럼 바로 복귀한다.
        print(
        ...
        if self._other_loaded_amr_waiting():
            print(
                "[1F 예약 인계 준비] AMR2 TO_MRI_LOADED 대기 확인 -> "
                "AMR1이 엘리베이터 옆 넓은 로비 대기점으로 이동합니다."
            )
            if not self.navigate("1층 복귀 예약 인계 대기점", RETURN_HOLD_1F, timeout=0.0):
                return False
            self.stop()
            self.spin_for(0.5)

            # 이동 중 AMR2 미션이 종료됐다면 불필요한 인계를 하지 않고 AMR1이
            # 기존 예약을 유지한 채 병실로 복귀한다.
            if self._other_loaded_amr_waiting():
                self.release_loaded_route("RETURN_HOLD_1F_HANDOFF")
                handoff_result = self._wait_for_amr2_up_handoff()
                if handoff_result == "FAILED":
                    return False
                if handoff_result == "REACQUIRE":
                    if not self.acquire_loaded_route("FROM_MRI_LOADED"):
                        return False
                else:
                    print(
                        "[1F 복귀 통행] AMR2는 엘리베이터로 2층 이동 중입니다. "
                        "서로 다른 층/구간이므로 AMR1은 재예약 없이 1층 병실로 복귀합니다."
                    )
            else:
                print(
                    "[1F 예약 인계 생략] 대기점 도착 전에 AMR2 요청이 사라져 "
                    "AMR1이 기존 예약을 유지합니다."
                )

        if self.amr_id == "amr1":
            print(
                "[1F 골목 진입 정렬] 넓은 공간의 전용 waypoint에서 침대를 "
                "골목 첫 직선 방향과 평행하게 맞춥니다."
            )
        ...
        self.release_loaded_route("PATIENT_ROOM_DROPOFF_CLEAR")
