...
class CenterlineNavigator(Node):
    ...
    def __init__(self) -> None:
        ...
        defaults = {
            ...
            # Optional final-yaw precision for one mission waypoint. Segment turns
            # and every other destination keep the normal rotate_tolerance_rad.
            "precision_final_goal_enabled": False,
            "precision_final_goal_x": 0.0,
            "precision_final_goal_y": 0.0,
            "precision_final_goal_match_tolerance_m": 0.15,
            "precision_final_yaw_tolerance_rad": math.radians(1.0),
            "retry_delay_sec": 0.8,
        ...
        self.precision_final_goal_enabled = bool(
            self.get_parameter("precision_final_goal_enabled").value
        )
        self.precision_final_goal_x = float(self.get_parameter("precision_final_goal_x").value)
        self.precision_final_goal_y = float(self.get_parameter("precision_final_goal_y").value)
        self.precision_final_goal_match_tolerance = max(
            0.01, float(self.get_parameter("precision_final_goal_match_tolerance_m").value)
        )
        self.precision_final_yaw_tolerance = max(
            0.005, float(self.get_parameter("precision_final_yaw_tolerance_rad").value)
        )
    ...
    def _rotation_tolerance_for_mode(self, mode: str) -> float:
        """Use tighter yaw tolerance only at the configured final waypoint."""
        effective_mode = mode
        if mode == "traffic_resume":
            effective_mode = self.interrupted_rotation_mode
        if (
            effective_mode == "final"
            and self.precision_final_goal_enabled
            and not self.keep_arrival_heading
            and self.final_goal_msg is not None
        ):
            goal = self.final_goal_msg.pose.position
            if math.hypot(
                goal.x - self.precision_final_goal_x,
                goal.y - self.precision_final_goal_y,
            ) <= self.precision_final_goal_match_tolerance:
                return self.precision_final_yaw_tolerance
        return self.rotate_tolerance
    ...
    def _begin_rotation(self, target_yaw: float, mode: str) -> None:
        ...
        tolerance = self._rotation_tolerance_for_mode(mode)
        self.get_logger().info(
            f"회전 시작({mode}): 목표 yaw={self.rotate_target_yaw:.4f}rad, "
            f"허용오차={math.degrees(tolerance):.2f}도, "
    ...
    def _rotation_tick(self) -> None:
        ...
        tolerance = self._rotation_tolerance_for_mode(self.rotate_mode)
