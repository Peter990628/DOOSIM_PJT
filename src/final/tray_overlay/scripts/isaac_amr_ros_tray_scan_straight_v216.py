...
from final_scene_staff import install_final_scene_staff_sync
...
def main() -> int:
    ...
    if bool(CFG.get("final_scene_staff", {}).get("enabled", False)):
        print("[V2.16.6 STAFF BARRIER] completing final staff before physics runtime", flush=True)
        install_final_scene_staff_sync(
            stage, PROJECT_ROOT, CFG.get("final_scene_staff", {}),
            CFG.get("hospital_staff", {}), CFG.get("fixed_mri_target", {}),
            CFG.get("cooperative_auto_transport", {}), APP.update, timeout_sec=120.0,
        )
        print("[V2.16.6 STAFF BARRIER PASS] no background staff Stage edits remain", flush=True)
    else:
    ...
    # V2.16 BACKUP ONLY - deterministic scan-ready start.
    # Both AMRs keep the exact final bay Y and yaw. They are moved ONLY backward
    # along cart-local -X far enough for the front cameras to see the ArUco gate.
    scan_demo_cfg = dict(CFG.get("scan_straight_demo", {}))
    if bool(scan_demo_cfg.get("enabled", False)):
        cart_len = float(cart_controller.meta.get("length", 2.20))
        dock_y = float(cart_controller.meta.get("dock_y", 0.425))
        standoff = float(scan_demo_cfg.get("marker_view_standoff_from_front_m", 0.75))
        start_local_x = -0.5 * cart_len - standoff
        cart_m = cart_controller._cart_matrix()
        cart_forward = cart_m.TransformDir(Gf.Vec3d(1, 0, 0))
        cart_yaw_deg = math.degrees(math.atan2(float(cart_forward[1]), float(cart_forward[0])))
        print(
            f"[V2.16 SCAN START GEOMETRY] cart_length={cart_len:.3f}m "
            f"front_x={-0.5*cart_len:+.3f}m standoff={standoff:.3f}m "
            f"AMR_start_local_x={start_local_x:+.3f}m "
            f"straight_to_dock={abs(start_local_x-float(cart_controller.meta.get('dock_x',0.0))):.3f}m"
        )
        for i, controller in enumerate(controllers[:2]):
            local = Gf.Vec3d(start_local_x, dock_y if i == 0 else -dock_y, 0.0)
            wp = cart_m.Transform(local)
            cart_controller._set_amr_pose(controller, float(wp[0]), float(wp[1]), cart_yaw_deg)
            try:
                controller._set_lift_raised(False, 0.0)
            except Exception:
                pass
            controller.halt()
            print(
                f"[V2.16 SCAN READY] AMR{i+1}: world=({float(wp[0]):+.3f},{float(wp[1]):+.3f}) "
                f"yaw={cart_yaw_deg:+.1f}deg local_x={start_local_x:+.3f} "
                f"local_y={(dock_y if i==0 else -dock_y):+.3f}"
            )
        cart_controller.attached = False
        cart_controller.pending = False
        print("[V2.16 SCAN READY PASS] DETACHED start; ArUco -> straight-only insertion -> ATTACH")
    ...
    try:
        while APP.is_running():
            ...
            if now_aux - cart_status_last >= float(CFG.get("scan_straight_demo", {}).get("cart_status_publish_period_s", 0.4)):
                cart_m = cart_controller._cart_matrix()
