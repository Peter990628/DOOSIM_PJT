...
class Nav2Bridge:
    ...
    @staticmethod
    def _extract_horizontal_scan(depth: np.ndarray, azimuth: np.ndarray, expected_samples: int) -> tuple[np.ndarray, np.ndarray | None]:
        """Return one horizontal revolution regardless of PhysX array orientation.

        Isaac Sim 5.1 can expose a single vertical channel as either (H, 1) or
        (1, H), while older code assumed the first axis was vertical.  With
        (720, 1) that assumption collapsed the scan to one ray.  This helper
        identifies the horizontal axis from the configured sample count /
        azimuth length, then takes the middle sample across every other axis.
        """
        d = np.asarray(depth, dtype=np.float32)
        a = np.asarray(azimuth, dtype=np.float32)
        if d.size == 0:
            return np.empty((0,), dtype=np.float32), None

        # Remove singleton dimensions first; (720,1) and (1,720) become 720.
        d = np.squeeze(d)
        if d.ndim == 0:
            d = d.reshape(1)

        az_flat = np.squeeze(a).reshape(-1) if a.size else np.empty((0,), dtype=np.float32)
        az_len = int(az_flat.size)

        if d.ndim == 1:
            ranges = d.reshape(-1)
        else:
            shape = tuple(int(v) for v in d.shape)
            candidates = list(range(d.ndim))
            # Prefer an axis matching azimuth samples, then configured ~720 samples.
            def score(axis: int) -> tuple[int, int, int]:
                n = shape[axis]
                az_penalty = 0 if az_len > 1 and n == az_len else 1
                expected_penalty = abs(n - expected_samples) if expected_samples > 1 else 0
                return (az_penalty, expected_penalty, -n)
            h_axis = min(candidates, key=score)
            moved = np.moveaxis(d, h_axis, -1)
            index = tuple(int(n // 2) for n in moved.shape[:-1]) + (slice(None),)
            ranges = np.asarray(moved[index], dtype=np.float32).reshape(-1)

        angles = None
        if az_len == ranges.size and az_len > 1:
            angles = az_flat.astype(np.float64)
        elif a.size > 1 and np.squeeze(a).ndim > 1:
            aa = np.squeeze(a)
            shape = tuple(int(v) for v in aa.shape)
            matches = [i for i, n in enumerate(shape) if n == ranges.size]
            if matches:
                h_axis = matches[0]
                moved = np.moveaxis(aa, h_axis, -1)
                index = tuple(int(n // 2) for n in moved.shape[:-1]) + (slice(None),)
                candidate = np.asarray(moved[index], dtype=np.float64).reshape(-1)
                if candidate.size == ranges.size:
                    angles = candidate
    ...
    def _publish_scan(self, sim_time: float) -> None:
        try:
            ...
            res_deg = float(self.cfg.get("lidar", {}).get("horizontal_resolution_deg", 0.5))
            expected_samples = max(2, int(round(360.0 / max(1e-6, res_deg))))
            ranges, angles = self._extract_horizontal_scan(depth, azimuth, expected_samples)
            if ranges.size == 0:
                return
            ...
            if angles is not None and angles.size == ranges.size:
                ...
                angle_min = -math.pi
                # LaserScan convention: N samples span [min, min+(N-1)*inc].
                angle_increment = float((2.0 * math.pi) / max(1, len(ranges)))
                angle_max = float(angle_min + angle_increment * max(0, len(ranges) - 1))
            ...
            # Runtime sanity line once the full revolution is really present.
            if not getattr(self, "_scan_shape_reported", False):
                span_deg = math.degrees(angle_increment * max(0, len(clean_ranges) - 1))
                print(
                    f"[NAV2 LIDAR 360 READY] {self.scan_topic} rays={len(clean_ranges)} "
                    f"expected≈{expected_samples} span≈{span_deg:.1f}deg raw_shape={tuple(depth.shape)}"
                )
                self._scan_shape_reported = True
