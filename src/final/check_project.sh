...
assert "name_only_threshold = 0.10" in ocr_source
assert "strong_wrong_name_threshold = 0.80" in ocr_source
assert '"name": "",' in ocr_source
ocr_params = (
    Path(sys.argv[2]).parents[1] / "ros2_ws/src/hospital_ocr_bridge/config/ocr_params.yaml"
).read_text(encoding="utf-8")
assert "verification_score_threshold: 0.10" in ocr_params
...
print("[OK] Domain 115, 1F elevator +90deg alignment and traffic pause wiring")
PY
